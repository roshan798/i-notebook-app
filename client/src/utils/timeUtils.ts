export const getRelativeTime = (createTimeDate: string | number | Date): string => {
    const now = new Date(); // Get the current date and time
    const createdAtDate = new Date(createTimeDate); // Parse the input date
    const diffInSeconds = Math.floor((now.getTime() - createdAtDate.getTime()) / 1000); // Calculate the difference in seconds
